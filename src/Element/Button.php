...
class Button implements ElementInterface
{
    ...
    public  function get() :array{
        ...
        if($this->type === 'web_url') $data['url'] = $this->value;

        if($this->type === 'postback' || $this->type === 'phone_number') $data['payload'] = $this->value;

        if($this->type === 'share_contents'){
            if($this->value) $data['share_contents'] = $this->value;
            unset($this->title);
        }
