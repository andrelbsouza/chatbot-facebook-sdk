...
class MenuManager
{
    ...
    private function mountCallToAction($parent_id = 0) {
        ...
        foreach ($this->call_to_actions as $action) {
            if ($action['parent_id'] === $parent_id)  $result[] = $this->mountAction($action);
        }
        return $result;
    }
    private function mountAction($action)
    {
        ...
        if ($action['type'] === 'postback') $data['payload'] = $action['value'];
        if ($action['type'] === 'web_url') $data['url'] = $action['value'];
        if ($action['type'] === 'nested') $data['call_to_actions'] = $this->mountCallToAction($action['id']);
